...
- **Crash recovery on startup (slice 4.E + §11.4 first cut):** `app/storage/session_recovery.py` marks unfinished sessions `DIRTY` in `session.json`, validates prior `recording/<feed_id>/` files via `cv2.VideoCapture`, quarantines corrupt segments to `<session>/quarantine/<feed_id>/`, and inserts `dirty` rows for valid in-progress files that lost their finalize step. `app/ui/recovery_dialog.py` then surfaces each dirty session in a modal Finalize/Discard prompt before any new session is created. (Resume — continuing the same `session_id` — is a follow-up slice.)
