...
- **Crash recovery on startup (slice 4.E):** `app/storage/session_recovery.py` marks unfinished sessions `DIRTY` in `session.json`, validates prior `recording/<feed_id>/` files via `cv2.VideoCapture`, quarantines corrupt segments to `<session>/quarantine/<feed_id>/`, and inserts `dirty` rows for valid in-progress files that lost their finalize step. `ApplicationCoordinator.initialize` runs this before creating a new session
