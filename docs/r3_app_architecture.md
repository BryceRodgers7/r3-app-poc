...
- Rebuild the referee-window transport row to match the spec: Play/Pause, 2x, 1/2x, 1/4x, 1/8x, Rewind 5s, Step ◀ / Step ▶, **inertia jog wheel** (mouse-drag, 1°/frame, momentum coast on release). Remove Replay Play and Jump to Live (the operator's Challenge button drives both behaviors now). All transport (buttons + wheel) is disabled outside an active challenge — see `window-requirements.md` §Referee. Add per-window camera show/hide ribbon + play-number badge (bottom-left).
...
| Referee window controls | `RefereeControlsWidget`: Pause, Rewind 10s, Replay Play, Slow 1/2x, Slow 1/4x, Step ◀, Step ▶, Jump to Live. (Phase 13.A.) | Play/Pause, 2x, 1/2x, 1/4x, 1/8x, Rewind 5s, Step ◀, Step ▶, **inertia jog wheel** (1°/frame, mouse-drag, momentum coast on release). All transport disabled except while a challenge is active. Replay Play and Jump to Live are removed — the operator's Challenge button replaces "jump to last play and pause." |
...
| Challenge / lockout | No concept of clip-bounded replay. `_resolve_rewind_target_locked` clamps against `available_session_time_range()` (full recording extent). | New `PlaybackController.set_clip_bounds(start_ns, end_ns)` / `clear_clip_bounds()` API. While bounds are set, every seek primitive (Rewind, Step, jog wheel, rate-change auto-snap) clamps to the range and snaps-to-PAUSED at either edge. Bounds drive the lockout described in `window-requirements.md` §Challenge. Additionally, the entire referee transport row is disabled outside an active challenge — see 14.F. |
...
Phase 14 lands in six slices. 14.A is the schema + manager refactor (everything else builds on the new vocabulary). 14.B and 14.C are independent UI slices and can land in either order; 14.D depends on 14.A + 14.B (challenge button exists) + 14.C (referee window has a place to render the lockout state). 14.E is the post-process modal — independent of 14.D and could land at any point after 14.B. 14.F (referee jog wheel + transport gating) depends on 14.D — it reuses the `challenge_state_changed` signal — and replaces the slider portion of 14.C.

**Status (resume here next session):** ✅ 14.A–14.E committed. ⬜ 14.F pending — start with `JogWheel` widget (independent of any other slice; only consumer is the referee `MainWindow`).
...
- **Lockout clear** — `mark_next_play` / `mark_timeout` / `stop_game` all call `self._referee_playback_controller.clear_clip_bounds()` if a fence is installed, then `jump_to_live()` to snap the referee back to the live edge, then emit `challenge_state_changed(active=False)`. The referee window's purpose is to watch live by default and only review on demand during a challenge; auto-returning to live keeps the operator-driven game flow uninterrupted (Phase 14.F.1 — originally Phase 14.D specified "stay paused at the current position" but field test showed the referee needed to manually rejoin live every time, confusing during fast play).
...
**14.F — Referee jog wheel + challenge-gated transport.**

Motivation: Phase 14.C shipped a horizontal `ScrubberSlider` on the referee window. Field test confirmed this does not match the requirement in [`window-requirements.md`](window-requirements.md) line 60 — the spec called for an "inertia wheel ... 1 degree = 1 frame," not a slider, and the spec also implies (lines 54-57) that the referee transport only makes sense during an active challenge. 14.F aligns the implementation with the original spec.

- **Remove `ScrubberSlider`.** Delete `app/ui/scrubber_slider.py`. Strip every reference from `app/ui/main_window.py` (construction at `controls_role == "referee"`, the `layout.addWidget(self.scrubber)` insertion in the referee layout branch, the `seek_to_session_time_requested.connect(...)` wire-up, and the `update_position(...)` call inside `_render_state`). Update `tests/test_referee_controls_phase_14c.py` — drop any slider-specific tests, the wheel covers the same controller surface.
- **New `JogWheel` widget** — `app/ui/jog_wheel.py`. Circular `QWidget` with `paintEvent` rendering the dial (a circle with tick marks and a rotating indicator). Public API:
  - `seek_by_frames_requested = Signal(int)` — positive = forward, negative = backward. Emitted as the wheel rotates, batched per Qt event (avoid flooding the controller with sub-frame deltas).
  - `set_enabled(active: bool)` — disables drag input and visually greys the wheel.
  - Frame size comes from `settings.replay_frame_step_count` (default frame step) or a fixed 1-frame quantum if the existing constant doesn't apply. One degree of wheel rotation → one frame of seek, per the spec.
  - Internal state: current angular position (display only), angular velocity (radians/s), inertia decay timer.
- **Interaction model — mouse drag only:**
  - `mousePressEvent` captures the angle of the cursor relative to the wheel center. If the click is outside the wheel's bounding circle, ignore.
  - `mouseMoveEvent` computes the delta angle from the last position, advances the wheel's rotational state, emits `seek_by_frames_requested(int(delta_degrees))` for the integer-frame portion (carrying the fractional remainder forward so a slow drag still seeks cleanly).
  - `mouseReleaseEvent` snapshots the recent angular velocity (last ~100ms) and starts the inertia decay: `QTimer` at ~60Hz applies the rotation delta to the wheel state, emits frame-seeks, and decays velocity by an exponential factor (`velocity *= 0.95` per tick) until below a threshold (~5°/s), then stops the timer.
  - During drag and during coast, the wheel re-paints continuously so the user sees rotational feedback. After coast ends, the wheel stays at its final angle (no spring-back).
  - No keyboard, scroll wheel, or touch input in this slice. Mouse drag only.
- **Wiring on the referee `MainWindow`:**
  - Construct `JogWheel` instead of `ScrubberSlider` in the `controls_role == "referee"` branch.
  - Place it in the referee layout where the slider was (between the video panel and the transport buttons).
  - Connect `seek_by_frames_requested` to a new coordinator method (or directly to `self._controller.step_frames(n)` — the existing primitive already does the right thing including clip-bounds clamping).
  - `_render_state` no longer pushes a position into the wheel — the wheel is a pure seek source, not a position display (unlike the slider, which had to track playback position to show the handle).
- **Challenge-state gating on the entire referee transport:**
  - `ApplicationCoordinator.signals.challenge_state_changed(bool)` already exists (Phase 14.D). Subscribe the referee `MainWindow` to it.
  - On `challenge_state_changed(True)`: enable every transport control (Play/Pause, 2x, 1/2x, 1/4x, 1/8x, Rewind 5s, Step ◀, Step ▶, jog wheel).
  - On `challenge_state_changed(False)`: disable all of them.
  - **Initial state at window construction must be disabled** — no challenge is active at startup. Do not rely on the first `challenge_state_changed(False)` emission to set the disabled state; set it explicitly in `MainWindow.__init__`.
  - The play-number badge color flip (Phase 14.C) keeps its existing wiring — that's the visual confirmation that the transport is now live.
  - Add a new `RefereeControlsWidget.set_transport_enabled(active: bool)` method that owns the enable/disable for every button. Mirrors the `set_recording_state` ownership-split lesson from 14.B follow-up (3) — keep this concern out of any other state setter to avoid the flicker race that bit Time-out / Challenge before.
- **Tests** — `tests/test_jog_wheel.py` (new): mouse-drag emits `seek_by_frames_requested` with the right sign and magnitude; release triggers inertia coast (use a fake timer to step the decay); coast stops below threshold; `set_enabled(False)` blocks input. `tests/test_referee_controls_phase_14f.py` (new): referee `MainWindow` starts with transport disabled; `challenge_state_changed(True)` enables every transport widget including the wheel; `challenge_state_changed(False)` disables them all. The existing 14.C transport-button tests stay — they exercise the buttons' signals, which the gating doesn't change.

**Out of scope for 14.F:** keyboard / scroll-wheel input on the jog wheel (the spec is mouse-drag-only for this slice). Touch input. Any change to `PlaybackController` primitives — `seek_to_session_time` and `step_frames` already do the right thing including clip-bounds clamping (Phase 14.D).
