...
* There will also be an inertia wheel which allows the referee to seek the video within the current clip. Whenever this wheel turns 1 degree, the video feed should seek 1 frame in that direction (forward or backward). If there is a more standard implementation of a seek-wheel, we can go with those requirements instead.
