...
- **Replay query goes through `RecordingSegmentReplayStore`** (slice 4.C). It enforces §10.4 / §6.6 (replay only while `recording_state == RECORDING`; only `state == "complete"` segments are returned) and resolves a target PTS to a `(segment_file, offset)` location. Actual segment-file decoding into the operator output is **slice 4.C.tail** (deferred); `PlaybackController` currently runs the replay state machine + clock but doesn't yet render rewound frames — see the TODOs in `playback_controller.py`.
...
- `app/storage/` — filesystem layout (`file_manager`), SQLite metadata (`metadata_db`, with the `segments` table), session lifecycle (`session_manager`), in-memory segment index (`segment_index`), replay query layer (`segment_replay_store`).
