...
- **Play markers (Phase 7.H — design locked, implementation pending).** Every moment of a recording belongs to a play; there is no "between plays" state. "Start game recording" implicitly opens **Play #1**. The "Next Play" button (replacing the slice 4.D `advance_short_segments` no-op stub) closes the current play and opens the next — incrementing a per-game `play_number`. "Stop game recording" closes the currently-open play. A new "Replay Play" transport button seeks to the currently-open play's `start_session_time_ns`. Plays live in a `plays` SQLite table with `UNIQUE(session_id, game_subdir, play_number)`. Crash recovery auto-closes any open play (sets `auto_closed_on_crash=TRUE`). Plays are operator-scoped (one sequence per game across all feeds), not feed-scoped. Phase 8.D writes a `<game>/plays.json` sidecar from this table during post-session processing.
